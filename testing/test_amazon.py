...
class TestAnsibleAmazonOutput (unittest.TestCase, TestAnsibleProvider):
    ...
    def check_host_of_software_component(self, playbook):
        ...
        for i in range(len(tasks)):
            if tasks[i].get('ec2_instance', None) != None:
                ...
                self.assertEqual(tasks[i + 2].get('set_fact', {}).get('group'),
                                 self.NODE_NAME + '_instance_public_address')
        ...
        self.assertEqual(tasks[1].get('include', None),
                         "/tmp/clouni/artifacts/testing/examples/ansible-server-example.yaml")
    ...
    def check_nodes_interfaces_operations(self, plays, testing_value):
        ...
        for i in range(2, 5):
            ...
            self.assertEqual(plays[i]['tasks'][1].get('include'),
                             '/tmp/clouni/artifacts/testing/examples/ansible-operation-example.yaml')
