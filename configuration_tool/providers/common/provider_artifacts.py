...
from configuration_tool.configuration_tools.ansible.runner.runner import run_ansible
...
from configuration_tool.common.tosca_reserved_keys import PARAMETERS, VALUE, EXTRA, SOURCE, DEFAULT_ARTIFACTS_DIRECTORY, \
    ANSIBLE
import yaml
import os
...
def execute(new_global_elements_map_total_implementation, is_delete, target_parameter=None, grpc_cotea_endpoint=None):
    if not is_delete:
        ...
        if grpc_cotea_endpoint:
            ...
            return run_ansible(new_ansible_tasks, grpc_cotea_endpoint, {}, {}, 'localhost', target_parameter=target_parameter,
                               ansible_library=ansible_library)  # add a variable for the default host
    return None